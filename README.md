...
![Version](https://img.shields.io/badge/version-2.2.0-0060B0?style=for-the-badge)
...
[Install](#-install) · [Supported AIs](#-supported-ai-platforms) · [Browsers](#-browser-support) · [Build from source](#-build-from-source) · [Privacy](#-privacy) · [License](#-license)
...
- **Loads with the bundled Firefox manifest keys (Gecko):** Firefox, Firefox Developer/Nightly, Zen, Floorp, LibreWolf, Waterfox, Mullvad Browser, Tor Browser*. *(Tor disables many web APIs by design; expect reduced functionality.)*
...
```bash
# from the repo root
./build.sh            # outputs dist/airadar-extension-v<version>.zip
```

The script copies `extension/` into `dist/`, strips dev cruft, and zips it for store upload or sharing. See **[CONTRIBUTING.md](CONTRIBUTING.md)** for the dev workflow and optional minification.
...
│  ├─ manifest.json           ← MV3 manifest (Chromium + Firefox keys)
...
│  └─ vendor/                 ← browser-polyfill shim + tokenizer data
